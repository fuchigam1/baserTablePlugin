 ...
 * @package Table.Model
 */
class TableComment extends TableAppModel {
 ...
 * @access public
 */
	public $belongsTo = array('TablePost' => array('className' => 'Table.TablePost',
 ...
 */
	public function add($data, $contentId, $postId, $commentApprove) {
		if (isset($data['TableComment'])) {
			$data = $data['TableComment'];
		}
		...
		$data['table_post_id'] = $postId;
		$data['table_content_id'] = $contentId;
		...
		$data['no'] = $this->getMax('no', array('table_content_id' => $contentId)) + 1;
		$this->create($data);
