...
<table cellpadding="0" cellspacing="0" class="list-table" id="ListTable">
	...
	<div>
		<?php if ($newCatAddable): ?>
			<?php $this->BcBaser->link($this->BcBaser->getImg('admin/btn_add.png', array('width' => 69, 'height' => 18, 'alt' => '新規追加', 'class' => 'btn')), array('action' => 'add', $tableContent['TableContent']['id'])) ?>
		<?php endif ?>
	</div>
...
<tbody>
	<?php if (!empty($dbDatas)): ?>
		...
		<?php foreach ($dbDatas as $data): ?>
			<?php
			$rowIdTmps[$data['TableCategory']['depth']] = $data['TableCategory']['id'];
			// 階層が上がったタイミングで同階層よりしたのIDを削除
			if ($currentDepth > $data['TableCategory']['depth']) {
				$i = $data['TableCategory']['depth'] + 1;
				while (isset($rowIdTmps[$i])) {
					unset($rowIdTmps[$i]);
					$i++;
				}
			}
			$currentDepth = $data['TableCategory']['depth'];
			$rowGroupId = array();
			foreach ($rowIdTmps as $rowIdTmp) {
				$rowGroupId[] = 'row-group-' . $rowIdTmp;
			}
			$rowGroupClass = ' class="depth-' . $data['TableCategory']['depth'] . ' ' . implode(' ', $rowGroupId) . '"';
			?>
			<?php $currentDepth = $data['TableCategory']['depth'] ?>
			<?php $this->BcBaser->element('table_categories/index_row', array('data' => $data, 'rowGroupClass' => $rowGroupClass)) ?>
		<?php endforeach; ?>
	<?php else: ?>
		<tr>
